...
add_filter( 'et/customizer/add/fields/header-builder', function ( $fields ) {
    ...
    $args['disable_customizer_header_builder'] = array(
        ...
        'default' => '<span class="customize-control-title">' . esc_html__( 'Enable Customizer header builder', 'xstore-core' ) . '</span><span class="customize-control-kirki-toggle flex"><input class="screen-reader-text" id="etheme-disable-default-header" name="etheme-disable-default-header" type="checkbox"><span class="switch" data-text-on="'.esc_attr__( 'On', 'xstore' ).'" data-text-off="'. esc_attr__( 'Off', 'xstore' ). '"></span><input type="hidden" name="nonce_etheme-switch_default" value="'.wp_create_nonce( 'etheme_switch_default' ).'"><input type="hidden" name="nonce_etheme_header-presets" value="'.wp_create_nonce( 'etheme_header-presets' ).'"></span>',
    );
//	$args = array(
